		...
		class fLoader extends Hls.DefaultConfig.loader {
			constructor(config) {
				...
				this.load = function (context, config, callbacks) {
					...
					for (i = count; i < USERJSON.length; i++) {
					...
					if (USERJSON[count].URL == "Server") {
						...
						//アクセス先のURLを動的変更
						context.url = `./hls/${USERJSON[count].dirName}/${USERJSON[count].dirName}_${USERJSON[count].tsFileName}`;
						if (true) {
							var onSuccess = callbacks.onSuccess;
							callbacks.onSuccess = function (response, stats, context) {
								//DOM表示
								$("#info").html( $("#info").html() + context.url + "<br>")
